...
export const Admin = () => {
  ...
  const handleSaveProject = (newProject: Project) => {
    ...
    if (editingProject) {
      ...
      const isCustom = currentCustom.some(
        (p: Project) => p.id === newProject.id,
      )

      if (isCustom) {
        updatedCustom = currentCustom.map((p: Project) =>
  ...
  const handleReply = (id: number, reply: string) => {
    ...
    const updatedPosts: QAPost[] = qaPosts.map(post => {
      if (post.id === id) {
        const newReply = {
          id: `reply-${Date.now()}`,
    ...
    <div className="min-h-screen bg-sage-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 md:gap-4 mb-8 border-b border-sage-200 overflow-x-auto">
          ...
          >
            <span className="flex items-center gap-1.5 md:gap-2">
              <MessageSquare className="w-4 h-4 md:w-5 md:h-5" /> 문의게시판
              관리
            </span>
            {activeTab === 'qa' && (
              <motion.div
            ...
            >
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-sage-100 overflow-hidden overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead className="bg-sage-50 text-sage-700 text-xs font-bold uppercase tracking-wider border-b border-sage-200">
                    <tr>
                      <th className="p-4 w-16 text-center whitespace-nowrap">
                        No
                      </th>
                      <th className="p-4 w-24 text-center whitespace-nowrap">
                        상태
                      ...
                      <th className="p-4 w-20 text-center whitespace-nowrap">
                        관리
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-sage-100">
                    {qaPosts.length === 0 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="p-12 text-center text-sage-400"
                        >
                          등록된 문의글이 없습니다.
                        </td>
                      </tr>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-sage-50 rounded-xl p-6 mt-8">
                  ...
                  <form
                    onSubmit={e => {
                      e.preventDefault()
                      handleReply(selectedPost.id, replyContent)
                    }}
                    className="mt-4"
                  >
                    <textarea
...
const WritePostModal = ({
  ...
  const handleReorderSpaces = (newOrder: Project['spaces']) => {
    setFormData(prev => ({
      ...prev,
      spaces: newOrder,
    }))
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
                            ...
                            {spaceImages.map((img, i) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setSpaceImages(
                                      spaceImages.filter((_, idx) => idx !== i),
                                    )
                                  }
          ...
          <div className="pt-4 border-t flex justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-sage-800 text-white font-bold rounded-lg hover:bg-sage-900"
            >
              {initialData ? '수정 완료' : '프로젝트 등록하기'}
            </button>
          </div>
        </form>
      </motion.div>
